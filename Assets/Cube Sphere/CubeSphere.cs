...
public class CubeSphere : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        for (int i = 0; i < vertices.Length; i++)
        {
            float scale = Mathf.Clamp(Mathf.Abs(Mathf.Sin(displacedVertices[i].y * axisScale - point)) + minRadius, 0.0f, maxRadius);
            displacedVertices[i].x = vertices[i].x * scale;
            displacedVertices[i].z = vertices[i].z * scale;
        }
